defmodule AoC2023.Day17.Part1 do
  ...
  defp min_heat_loss(map) do
    ...
    finish = fn {{pos, _}, _, _} -> pos == {max_x, max_y} end
    cost = fn _, {{pos, _}, _, _} -> map[pos] end
    ...
    |> Enum.map(fn {{pos, _}, _, _} -> map[pos] end)
    |> Enum.sum()
  end

  defp move({x, y}, {dx, dy}), do: {{x + dx, y + dy}, {dx, dy}}

  defp prune(next, visited, max_x, max_y) do
    next
    |> Enum.filter(fn {{{x, y}, _}, _, _} -> x in 0..max_x && y in 0..max_y end)
    |> Enum.reject(fn {current, _, _} -> current in visited end)
    |> Enum.reject(fn {_, dirs, _} ->
      dirs |> Enum.frequencies() |> (fn d -> Map.values(d) == [4] end).()
    end)
  end
